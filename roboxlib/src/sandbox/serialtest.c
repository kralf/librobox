...
int serial_write(int fd, unsigned char* data, ssize_t num) {
  ssize_t num_written = 0;

  while (num_written < num) {
    ssize_t n;
    while ((n = write(fd, &data[num_written], num-num_written)) == 0);
    if (n > 0)
      num_written += n;
    else
      return -1;
  ...
  if (fd > 0) {
    ...
    if (!tcsetattr(fd, TCSANOW, &tio)) {
      ...
      while (!quit) {
        ...
        if (serial_write(fd, data, num) == num) {
          if (serial_read(fd, data, num) == num) {
            num_trans += num;
            ...
            fprintf(stderr, "\rtransferred %6d kB at %6d kBit/s", 
